...
class Listener(object):
    def __init__(self):
        ...
        self.current_arm_ring = None
        self.current_arm_peg = None
        self.rings = []
        self.pegs = []
        self.arm_rings = []
        self.arm_pegs = []
...
class GraspRing(smach.State):
    def __init__(self, listener):
        smach.State.__init__(self, outcomes=['at_ring', 'at_center'], input_keys=['arm'], output_keys=['arm'])
        self.listener = listener
    ...
    def execute(self, userdata):
        ...
        if self.listener.current_arm_ring == userdata.arm:
            userdata.arm = userdata.arm
            return 'at_ring'
        else:
            userdata.arm = self.listener.current_arm_ring
            return 'at_center'
...
class ExtractRing(smach.State):
    def __init__(self, listener):
        smach.State.__init__(self, outcomes=['reachable_peg', 'unreachable_peg'])
        self.listener = listener
    ...
    def execute(self, userdata):
        ...
        if self.listener.current_arm_ring == self.listener.current_arm_peg:
            return 'reachable_peg'
        else:
            return 'unreachable_peg'
...
class ReleaseRing(smach.State):
    def __init__(self, listener):
        smach.State.__init__(self, outcomes=['at_peg', 'at_center', 'finished'], input_keys=['arm'])
        self.listener = listener
    ...
    def execute(self, userdata):
        ...
        if userdata.arm != self.listener.current_arm_peg:
            return 'at_center'
        elif len(self.listener.rings) == 0:
            return 'finished'
        return 'at_peg'
...
def main():
    ...
    with sm_top:
        ...
        smach.StateMachine.add('GRASP_RING', GraspRing(listener), transitions={'at_ring': 'EXTRACT_RING', 'at_center': 'RELEASE_RING'})
        smach.StateMachine.add('EXTRACT_RING', ExtractRing(listener), transitions={'reachable_peg': 'MOVE_TO_PEG', 'unreachable_peg': 'MOVE_TO_CENTER'})
        ...
        smach.StateMachine.add('RELEASE_RING', ReleaseRing(listener), transitions={'at_peg': 'MOVE_TO_RING', 'at_center': 'MOVE_TO_PEG', 'finished': 'succeeded'})
