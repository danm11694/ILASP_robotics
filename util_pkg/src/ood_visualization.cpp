...
// MOD : ROS TIME 0 ---> ROS TIME NOW, ADDED CONVERSIONS
	  ...
	  is_init_pose_hist_pub(false), is_init_cloud_hist_pub(false)
{
	...
	centroid_marker = nh_.advertise<visualization_msgs::Marker>("/plane_centroid", 1, true);
...
ood_visualization::add_curr_cloud(PointCloudRGB::Ptr cloud)
{
	...
	pcl_conversions::toPCL(ros::Time(0),cloud->header.stamp); // now -> 0
  	// cloud->header.stamp = ros::Time::now();//ros::Time(0);	// ensure it does not get deleted by rviz
...
ood_visualization::add_hist_cloud_v1(PointCloudRGB::Ptr cloud)
{
	...
	rgbcloud_pub_vec.push_back(pc_pub);
  	...
  	cloud->header.frame_id = "/world";
  	pcl_conversions::toPCL(ros::Time(0),cloud->header.stamp); // now -> 0
  	//std::cout << "[ADD HIST CLOUD] TIME : " << cloud->header.stamp << std::endl;
  	// cloud->header.stamp = ros::Time::now();//ros::Time(0);	// ensure it does not get deleted by rviz
...
ood_visualization::add_hist_pose_v1(Eigen::Matrix<T,3,1> const &position, Eigen::Matrix<T,4,1> const &orientation)
{
	...
	pose_marker.header.stamp = ros::Time(0); //  now to 0	// ensure it does not get deleted by rviz
...
ood_visualization::add_hist_pose_v2(Eigen::Matrix<T,3,1> const &position, Eigen::Matrix<T,4,1> const &orientation)
{
	...
	ood_visualization::PoseM pose_marker;
	pose_marker.header.frame_id = "/world";
	pose_marker.header.stamp = ros::Time(0); // ::now() , myb ros::Time(0);	// ensure it does not get deleted by rviz
...
ood_visualization::init_pose_hist_pub()
{
	...
	pose_hist.header.frame_id = "/world";
	pose_hist.header.stamp = ros::Time(0);//now to 0 	// ensure it does not get deleted by rviz
...
ood_visualization::init_cloud_hist_pub()
{
	...
	cloud_hist.header.frame_id = "/world";
	pcl_conversions::toPCL(ros::Time(0),cloud_hist.header.stamp);  //now -> 0 
  	// cloud_hist.header.stamp = ros::Time::now();// ros::Time(0);	// ensure it does not get deleted by rviz
}
...
ood_visualization::add_centroids( const Eigen::MatrixXd & mat_3xN )
{
	...
	marray.header.frame_id = "/world";
	marray.header.stamp = ros::Time(0);// now to 0;	// ensure it does not get deleted by rviz
...
void
ood_visualization::add_centroid(const Eigen::Vector4f & vec_4, float scale, const std::string frame)
{
    ...
    marker.ns = "MarkerCentroid";
    marker.id = 1;
    marker.type = shape;
    marker.action = visualization_msgs::Marker::ADD;

    marker.pose.position.x = vec_4[0];
    marker.pose.position.y = vec_4[1];
    marker.pose.position.z = vec_4[2];
    marker.pose.orientation.x = 0.0;
    marker.pose.orientation.y = 0.0;
    marker.pose.orientation.z = 0.0;
    marker.pose.orientation.w = 1.0;

    marker.scale.x = scale; // (max[0]-min[0]);
    marker.scale.y = scale; // (max[1]-min[1]);
    marker.scale.z = scale; // (max[2]-min[2]);
	...
	centroid_marker.publish(marker);
}
										 ...
										 const Eigen::MatrixXf & colors )
{
	...
	{
		bboxes.markers[i].header.frame_id = "/world";
		bboxes.markers[i].header.stamp = ros::Time(0);// now to 0// ensure it does not get deleted by rviz
									 ...
									 const Eigen::VectorXd & obj_ids )
{
	...
	for( int i = 0; i < num_ids; ++i)
	{
		...
		ids.markers[i].header.frame_id = "/world";
		ids.markers[i].header.stamp = ros::Time(0);//now to 0;	// ensure it does not get deleted by rviz
		ids.markers[i].id = i;
...
{
	pcl_conversions::toPCL(ros::Time(0),cld_ptr->header.stamp); // now -> 0
  	// cld_ptr->header.stamp = ros::Time::now();// ros::Time(0);	// ensure it does not get deleted by rviz
	curr_view_pub.publish(*cld_ptr);
}

void 
ood_visualization::add_curr_view_rgb( pcl::PointCloud<pcl::PointXYZRGB>::Ptr & cld_ptr)
{
	pcl_conversions::toPCL(ros::Time(0),cld_ptr->header.stamp); // now -> 0
  	// cld_ptr->header.stamp = ros::Time::now();// ros::Time(0);	// ensure it does not get deleted by rviz
	curr_view_pub.publish(*cld_ptr);
