...
public:
                                                ...
                                                far_clip(3.50e+00)
    {
        ...
        private_nh_.param("pcd_topic", pcl_topic, std::string("/camera/depth/image"));
        private_nh_.param<bool>("old_plugin", old_plugin, true);

        color_pcl_pub_ = nh_.advertise<sensor_msgs::PointCloud2>(nh_.resolveName("/camera/points"), 10); // why 10
    ...
    void update()
    {
        ...
        else
        {
            ...
            if (!image_depth.empty() && !image_rgb.empty() && !K.empty())
            {
                ...
                pcd_utils::pointCloudFrom2D(image_depth, image_rgb, K, test3d2d);
            ...
            if (!image_depth.empty() && !image_rgb.empty() && !K.empty())
            {
                ...
                pcd_utils::pointCloudFrom2D(image_depth, image_rgb, K, mergedCloud);
                pcl::toROSMsg(*mergedCloud, cld_msg);
                cld_msg.header.frame_id = camera_color_frame;
                color_pcl_pub_.publish(cld_msg);
            }
